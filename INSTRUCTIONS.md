...
2. [C엔진] `main.py` 진입점에 `uvloop.install()`, 정규장 `gc.disable()` 강제.
...
1. [메모리 점유] 매매 프로세스 실행 시 `mlockall(MCL_CURRENT | MCL_FUTURE)`를 호출하여 메모리 스와핑을 원천 차단하라.
2. [실시간 우선순위] `os.sched_setscheduler`를 통해 리얼타임 우선순위(SCHED_FIFO)를 획득하여 OS 스케줄링 간섭을 배제하라.
...
# 6부: 자율 수리 루프
1. 완료 후 터미널에서 `./verify.sh`를 실행하여 자율 수리하라.
