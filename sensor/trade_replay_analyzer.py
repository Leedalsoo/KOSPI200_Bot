...
- 매 거래 체결(진입 및 청산) 시 진입/청산 사유, 센서 스냅샷, 대시보드 상태 스냅샷을 캡처
...
class TradeReplayAnalyzer:
    ...
    def capture_trade_event(
        ...
        """
        거래 발생 시 센서, 대시보드 상태, 규정 준수 및 AI 타이밍 분석 자동 생성 및 기록
        """
    ...
    def _check_rule_compliance(self, track_name: str, sensor: Dict[str, Any], state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        z_score = abs(sensor.get("zScore", 0.0))
    ...
    def _analyze_timing_optimization(
        ...
        vol = sensor_snapshot.get("activeVol", 1.0)
    ...
    def get_recent_records(self, count: int = 20) -> List[Dict[str, Any]]:
